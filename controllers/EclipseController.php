...
use app\models\forms\EclipseForm;
use app\models\observations\Eclipse;
use Yii;
...
class EclipseController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $searchModel->type = Observe::TYPE_ECLIPSE;
    ...
    public function actionUpdate($id)
    {
        ...
        if ($observe->load(Yii::$app->request->post())) {
            if ($observe->image == null) {
                $eclipseForm = new EclipseForm();
                $eclipseForm->image = UploadedFile::getInstance($observe, 'image');

                $eclipseForm->uploadImage($observe->id);
            }
            if ($observe->save()) {
     ...
     * @param integer $id
     * @return Eclipse the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Eclipse::findOne($id)) !== null) {
            return $model;
        }
